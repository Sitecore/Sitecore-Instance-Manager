...
namespace SIM.Base
{
  ...
  public static class Extensions
  {
    ...
    /// <summary>
    /// Calls the function preventing exception that is logged to the log file
    /// </summary>
    /// <typeparam name="T">
    /// Any class
    /// </typeparam>
    ...
    /// </param>
    /// <returns>
    /// The result of function executing or null if exception is happend
    /// </returns>
    [CanBeNull]
    ...
    public static IEnumerable<string> Extract(this string message, char startChar, char endChar, bool includeBounds)
    {
      ...
      while (true)
      {
        ...
        if (start < 0)
          yield break;
        Assert.IsTrue(start + 1 < length, "Cannot replace variables in the \"{0}\" string - line unexpectedly ends after {1} position".FormatWith(message, start));
        end = message.IndexOf(endChar, start + 1);
        Assert.IsTrue(end > start, "Cannot replace variables in the \"{0}\" string - no closing {1} character after {2} position".FormatWith(message, end, start));
        start += includeBounds ? 0 : 1;
        int len = end - start + (includeBounds ? 1 : 0);
        Assert.IsTrue(len > 0,
                      "Cannot replace variables in the \"{0}\" string - the string contains invalid '{{}}' statement".
                        FormatWith(message));
    ...
    /// <summary>
    /// Ensures that string's end is specific and not repeated.
    /// For example, EnsureEnd(@"C:\\", @"\") == @"C:\"
    /// </summary>
    ...
    /// <summary>
    /// Same as string.Substring, but if end > length then end = length
    /// </summary>
