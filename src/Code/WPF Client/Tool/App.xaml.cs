...
<configuration>
  <appSettings>
    ...
    <add key=""MessageURL"" value=""http://dl.sitecore.net/updater/1.1/sim/message.txt""/>
          ...
          if (!File.Exists(updaterFileName))
          {
            using (var stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("SIM.Tool" + @".Updater.exe"))
            {
              Assert.IsNotNull(stream, "stream");

              using (var fileStream = new System.IO.FileStream("Updater.exe", System.IO.FileMode.Create))
              {
                int len;
    ...
    private static Profile DetectProfile()
    {
      ...
      if (!FileSystem.Local.File.Exists(lic))
      {
        FileSystem.Local.File.Copy(instance.LicencePath, lic);
      }
