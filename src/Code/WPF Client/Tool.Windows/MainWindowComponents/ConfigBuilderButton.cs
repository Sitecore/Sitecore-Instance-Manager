...
namespace SIM.Tool.Windows.MainWindowComponents
{
  public class ConfigBuilderButton : IMainWindowButton
  {
    ...
    public static void RunConfigApp(string name, Window mainWindow, string param = null)
    {
      ...
      if (!FileSystem.Local.File.Exists(path) || (!string.IsNullOrEmpty(latestVersion) && FileVersionInfo.GetVersionInfo(path).ProductVersion != latestVersion))
      {
    ...
    private static void GetLatestVersion(string path)
    {
      try
      {
        ...
        FileSystem.Local.Directory.Ensure(folder);
        ...
        FileSystem.Local.Zip.UnpackZip(packageZipPath, folder);
      }
      catch (Exception ex)
      {
