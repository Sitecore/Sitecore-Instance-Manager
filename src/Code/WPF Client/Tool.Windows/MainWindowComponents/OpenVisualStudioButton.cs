// -----------------------------------------------------------------------
// <copyright file="OpenVisualStudioButton.cs" company="">
...
namespace SIM.Tool.Windows.MainWindowComponents
{
  public class OpenVisualStudioButton : IMainWindowButton
  {
    ...
    public void OnClick(Window mainWindow, Instance instance)
    {
      if (instance != null)
      {
        ...
        else if (paths.Length == 0)
        {
          ...
          if (is66 || is7x || is8)
          {
            var options = is71plus ? new[] { noThanks, yesAspNetMvc } : new[] { noThanks, yesAspNetWebforms, yesAspNetMvc };
            ...
            {
              if (!EnsureMvcEnabled(mainWindow, instance, version))
              {
                return;
              }
          ...
          Assert.IsTrue(!string.IsNullOrEmpty(path) && FileSystem.Local.File.Exists(path), "The Visual Studio files are missing");
    ...
    private bool EnsureMvcEnabled(Window mainWindow, Instance instance, string version)
    {
      ...
      if ((!FileSystem.Local.File.Exists(Path.Combine(instance.WebRootPath, "bin\\Sitecore.Mvc.dll")) || !FileSystem.Local.File.Exists(Path.Combine(instance.WebRootPath, "App_Config\\Include\\Sitecore.Mvc.config"))) && (version == "6.6" || version == "7.0"))
      {
        // suggest to install them
        const string yesEnableSitecoreMvc = "Yes, enable Sitecore MVC";
        const string noCancel = "No, cancel";
        var options2 = new[] { noCancel, yesEnableSitecoreMvc };
        ...
        if (!EnsureFramework4(mainWindow, instance))
        {
          return false;
        }

        if (!EnsureMvcConfigured(instance, version))
        {
          return false;
        }
    ...
    private bool EnsureMvcConfigured(Instance instance, string version)
    {
      ...
      PipelineManager.StartPipeline("installmodules", new InstallModulesArgs(instance, new[] { enableMvcProduct }), isAsync: false);

      if (!EnsureMvcForDmsConfigured(instance, version))
      {
        return false;
      }
    ...
    private bool EnsureMvcForDmsConfigured(Instance instance, string version)
    {
      ...
      if (FileSystem.Local.File.Exists(dmsConfigPath) && !FileSystem.Local.File.Exists(dmsMvcConfigPath))
      {
        ...
        PipelineManager.StartPipeline("installmodules", new InstallModulesArgs(instance, new[] { enableMvcForDmsProduct }),
                                      isAsync: false);
      }
