...
namespace SIM.Tool.Windows.UserControls.Install.Modules
{
  ...
  public partial class FilePackages : IWizardStep, ICustomButton, IFlowControl
  {
    ...
    public void CustomButtonClick()
    {
      ...
      if (openDialog.ShowDialog(Window.GetWindow(this)) == true)
      {
        foreach (string path in openDialog.FileNames)
        {
          ...
          FileSystem.Local.File.Copy(path, Path.Combine(ApplicationManager.FilePackagesFolder, fileName));
    ...
    private void Append(string folder)
    {
      ...
      IEnumerable<string> files = FileSystem.Local.Directory.GetFiles(folder, "*.zip", SearchOption.AllDirectories).Where(f => !f.ContainsIgnoreCase("Visual Studio") || f.ContainsIgnoreCase("Visual Studio " + ver));
