...
namespace SIM.Tool.Base.Plugins
{
  ...
  public class PluginManager
  {
    ...
    public static IEnumerable<Plugin> Plugins =
      ...
      new[] {"Plugins", ApplicationManager.PluginsFolder}.Where(FileSystem.Local.Directory.Exists)
        // get all 1-level nested folders
        .SelectMany(FileSystem.Local.Directory.GetDirectories)
    ...
    public static IEnumerable<Plugin> GetEnabledPlugins(Profile profile)
    {
      ...
      var plugins = profile.Plugins.Replace(workaround, "");
    ...
    private static void EnableStockPluginsOnce()
    {
      ...
      try
      {
        ...
        {
          var arr = @"Plugins\Publish Dialog|Plugins\LINQPad|Plugins\Log Analyzer|Plugins\Support Toolbox|Plugins\Support Package Generator|Plugins\Update License|Plugins\Multiple Deletion|Plugins\Sitecore Rocks|Plugins\Install MongoDB".Split('|');
          ...
          File.WriteAllText(agreementAcceptedFilePath, "");
        }
      }
      catch (Exception ex)
      {
