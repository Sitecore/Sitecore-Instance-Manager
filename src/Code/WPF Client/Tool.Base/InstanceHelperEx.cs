namespace SIM.Tool.Base
{
  ...
  public static class InstanceHelperEx
  {
    ...
    public static void OpenCurrentLogFile([NotNull] Instance instance, [NotNull] Window owner, [CanBeNull] string logFileType = null)
    {
      ...
      var files = FileSystem.Local.Directory.GetFiles(logs, pattern) ?? new string[0];
      var logFilePath = files.OrderByDescending(FileSystem.Local.File.GetCreationTimeUtc).FirstOrDefault();
      ...
      fileSystemWatcher.Created += (sender, args) =>
      {
        try
        {
          ...
          // magic begins
          process.Kill();
          logFilePath = files.OrderByDescending(FileSystem.Local.File.GetCreationTimeUtc).First();
    ...
    private static string GetLogFileTypes(Window owner, string logsFolderPath)
    {
      ...
      var files = FileSystem.Local.Directory.GetFiles(logsFolderPath, Pattern);
