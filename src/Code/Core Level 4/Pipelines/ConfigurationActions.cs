...
namespace SIM.Pipelines
{
  ...
  /// <summary>
  ///   Configuration Actions for installing modules
  /// </summary>
  public static class ConfigurationActions
  {
    ...
    {
      get { return new Credentials { UserName = "sitecore\\admin", Password = "b" }; }
    }
      ...
      [CanBeNull] IPipelineController controller = null, [CanBeNull] Dictionary<string, string> variables = null)
    {
      ...
      foreach (Product module in modules)
      {
        ...
        if (manifest == null)
        {
          SIM.Base.Log.Warn("The {0} doesn't have a manifest".FormatWith(module), typeof(ConfigurationActions));
          if (done != null)
          {
            done.Add(module);
          }
        ...
        {
          SIM.Base.Log.Warn("The {0} doesn't suites for {1}".FormatWith(module, instanceProduct), typeof(ConfigurationActions));
          if (done != null)
          {
            done.Add(module);
          }
        ...
        if (element == null)
        {
          SIM.Base.Log.Warn("Can't find rules root (the {0} element in the manifest of the {3} file){1}The manifest is: {1}{2}".FormatWith(xpath, Environment.NewLine, manifest.OuterXml, module.PackagePath), typeof(ConfigurationActions));
          if (done != null)
          {
            done.Add(module);
          }
    ...
    public static Database GetMainDatabase(Instance instance, SqlConnectionStringBuilder connectionString)
    {
      using (new ProfileSection("Get main database", typeof(ConfigurationActions)))
      {
        ...
        string[] firstOrderDatabaseNames = new[] { "core", "master", "web" };
        var localDBs =
    ...
    private static void AddDatabase([NotNull] Instance instance, [NotNull] IEnumerable<XmlElement> databases, [NotNull] Product module, SqlConnectionStringBuilder connectionString, IPipelineController controller)
    {
      ...
      foreach (XmlElement database in databases)
      {
        ...
        FileSystem.Local.Directory.AssertExists(databasesFolder);
          ...
          case "replace":
            {
              ...
              FileSystem.Local.File.WriteAllText(path, FileSystem.Local.File.ReadAllText(path).Replace(source, target));
              break;
            }
          case "replacevariables":
            {
              var text = FileSystem.Local.File.ReadAllText(path);
          ...
          case "append":
            {
              ...
              FileSystem.Local.File.WriteAllText(path, FileSystem.Local.File.ReadAllText(path).Replace(before, target + before));
              break;
            }
          case "move":
            {
              ...
              FileSystem.Local.Directory.DeleteIfExists(destFileName);
              FileSystem.Local.File.Move(path, destFileName);
              break;
            }
          case "copy":
            {
              ...
              string destFileName = Path.Combine(instanceRootPath, target.TrimStart('/'));
              FileSystem.Local.Directory.DeleteIfExists(destFileName);
              FileSystem.Local.File.Copy(path, destFileName);
              break;
            }
        }
      }
    ...
    private static IEnumerable<string> GetOptions(string typeName, string methodName, Instance instance)
    {
      ...
      return (IEnumerable<string>)method.Invoke(null, new object[] { instance });
    }
          ...
          case "append":
            {
              ...
              if (parentNode == null)
              {
                SIM.Base.Log.Warn("[InstallActions, Append] The {0} element isn't found".FormatWith(xpath), typeof(ConfigurationActions));
                break;
              }
          ...
          case "include":
            {
              ...
              FileSystem.Local.Zip.UnpackZip(module.PackagePath, Path.Combine(instance.WebRootPath, "app_config\\include"), filePath);
              break;
            }
          ...
          case "change":
            {
              ...
              {
                SIM.Base.Log.Warn("The xpath attribute is missing in the {0} instruction (outer xml: {1})".FormatWith(instruction.Name, instruction.OuterXml), typeof(ConfigurationActions));
                continue;
              }
              ...
              {
                SIM.Base.Log.Warn("Can't find the {0} element in the {1} file".FormatWith(xpath, config.FilePath), typeof(ConfigurationActions));
                continue;
              }
          ...
          case "delete":
            {
              ...
              {
                SIM.Base.Log.Warn("Can't find the {0} nodes in the {1} file".FormatWith(xpath, config.FilePath), typeof(ConfigurationActions));
                continue;
              }
              ...
              foreach (XmlElement targetElement in nodes.OfType<XmlElement>())
              {
                ...
                if (parent == null)
                {
                  SIM.Base.Log.Warn("Can't find the parent node of the {0} element of the {1} file".FormatWith(xpath, config.FilePath), typeof(ConfigurationActions));
                  continue;
                }
          ...
          case "rename":
            {
              ...
              if (!string.IsNullOrEmpty(fromFileName) && !string.IsNullOrEmpty(toFileName))
              {
                ...
                try
                {
                  if (!FileSystem.Local.File.Exists(fromPath) && FileSystem.Local.File.Exists(toPath))
                  {
                    Log.Warn("The moving does not seem to be needed", typeof(ConfigurationActions));
                    break;
                  }

                  FileSystem.Local.File.Move(fromPath, toPath);
                }
                catch (Exception ex)
                {
      ...
      XmlElement actionsElement)
    {
      ...
      {
        FileSystem.Local.Zip.UnpackZip(module.PackagePath, instance.WebRootPath);
      }
      ...
      foreach (XmlElement action in actions)
      {
        ...
        switch (actionName)
        {
          ...
          case "sql":
          {
            ...
            var sql = string.IsNullOrEmpty(file) ? action.InnerText : FileSystem.Local.File.ReadAllText(file);
    ...
    private static string GetDatabasesFolder(Instance instance, SqlConnectionStringBuilder connectionString, IPipelineController controller)
    {
      ...
      if (mainDatabase == null)
      {
        while (true)
        {
          ...
          if (!FileSystem.Local.Directory.Exists(databasesFolder))
          {
            if (controller.Confirm("The " + databasesFolder + " doesn't exist. Would you like to create the folder?"))
            {
              FileSystem.Local.Directory.CreateDirectory(databasesFolder);
              break;
            }
          }
          else
    ...
    ///   The file name. 
    /// </param>
    /// <param name="databasesFolder">
    ///   The databases folder. 
    /// </param>
    /// <param name="locationInPackage"></param>
    ...
    private static void ExtractDatabase(Product module, string fileName, string sourceFileName, string databasesFolder, string locationInPackage)
    {
      ...
      {
        using (var tmp = FileSystem.Local.Directory.GetTempFolder())
        {
    ...
    private static void ExtractDatabaseInner(string fileName, [CanBeNull] string sourceFileName, string databasesFolder, string packagePath, string tmpPath = null, string location = null)
    {
      ...
      {
        FileSystem.Local.Zip.UnpackZip(packagePath, tmpPath, packageZipFileName);
        ...
        try
        {
          ...
          FileSystem.Local.Zip.UnpackZip(packagePath, tmpPath, inPackageFilePath.Replace("\\", "/"));
          ...
          if (FileSystem.Local.File.Exists(source))
          {
            if (FileSystem.Local.File.Exists(target))
            {
              FileSystem.Local.File.Delete(target);
            }

            FileSystem.Local.File.Move(source, target);
            return;
          }
        }
        catch (IOException e)
      ...
      using (var tempFolder = FileSystem.Local.Directory.GetTempFolder())
      {
        try
        {
          if (string.IsNullOrEmpty(sourceFileName))
          {
            FileSystem.Local.Zip.UnpackZip(packagePath, databasesFolder, fileName);
          }
          else
          {
            FileSystem.Local.Zip.UnpackZip(packagePath, tempFolder.Path, sourceFileName);
          }
        }
        catch (IOException e)
        {
        ...
        if (!string.IsNullOrEmpty(sourceFileName))
        {
          ...
          FileSystem.Local.File.Move(sourceFilePath, destFilePath);
        }
      }
    }
