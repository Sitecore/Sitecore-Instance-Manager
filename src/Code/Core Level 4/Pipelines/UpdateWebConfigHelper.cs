...
namespace SIM.Pipelines.Install
{
  ...
  public static class UpdateWebConfigHelper
  {
    public static void Process([NotNull] string rootFolderPath, [NotNull] string webRootPath, [NotNull] string dataFolder)
    {
      ...
      {
        CreateIncludeFile(rootFolderPath, "UseServerSideRedirect.config", new NameValueCollection { { "RequestErrors.UseServerSideRedirect", "true" } });
      }
  ...
  <sitecore>
    <settings>";
      ...
      sb.Append(Postfix);
      FileSystem.Local.File.WriteAllText(includeFilePath, sb.ToString());
    }
  }
}
