...
{
  [UsedImplicitly]
  class ImportRegisterWebsite : ImportProcessor
  {
    protected override void Process(ImportArgs args)
    {
      ...
      ChangeAppPoolSettingsIfNeeded(args.temporaryPathToUnpack.PathCombine(ImportArgs.appPoolSettingsFileName), args);
      ChangeWebsiteSettingsIfNeeded(args.temporaryPathToUnpack.PathCombine(ImportArgs.websiteSettingsFileName), args);
      //
    ...
    private void ChangeWebsiteSettingsIfNeeded(string path, ImportArgs args)
    {
      ...
      //      
      args.siteName = CreateNewSiteName(InstanceManager.Instances, args.siteName);
      ...
      args.siteID = CreateNewID(args.siteID);
    ...
    private void ChangeAppPoolSettingsIfNeeded(string path, ImportArgs args)//should be executed before ChangeWebsiteSettingsIfNeeded
    {
        //need to change AppName
        XmlDocumentEx appPoolSettings = new XmlDocumentEx();
        appPoolSettings.Load(path);
        //
        args.appPoolName = CreateNewAppPoolName(args.appPoolName);
    ...
    public string CreateNewName(string[] names, string oldName)
    {
        ...
        while (!flag)
        {
            if ((from t in names where t == oldName select t).FirstOrDefault() == null)
                return oldName;
            else oldName += "_imported";
        }
        return null;
    }
    //
    public string CreateNewSiteName(IEnumerable<Instance> instances, string oldName)
    {
        ...
        while (!flag)
        {
            if ((from t in sitesNames where t == oldName select t).FirstOrDefault() == null)
                return oldName;
            else oldName += "_imported";
        }
        return null;
    ...
    public string CreateNewAppPoolName(string oldName)
    {
        ...
        while (!flag)
        {
            if ((from t in poolsNames where t == oldName select t).FirstOrDefault() == null)
                return oldName;
            else oldName += "_imported";
        }
        return null;
